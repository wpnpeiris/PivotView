   ...
   $.widget("unc.jbPivot", {
      ...
      _create: function() {
         ...
         //if (typeof this.options.data !== "undefined") {
            var data = DataCubeProxy.getInstance().getCuboid(this);
      ...
      reset: function() {
         ...
         //var dataFields = DataCubeProxy.getInstance().getFields();
         for (f in this.options.fields) {
         //for (f in dataFields) {
            if (this.options.fields.hasOwnProperty(f)) {
            //if (dataFields.hasOwnProperty(f)) {
               var fo = $.extend({
         ...
         for (i = 0; i < records.length; i++) {
            ...
            for (f = 0; f < this.fields.length; f++) {
               if (this.fields[f] !== null) {
                  tmp = this.fields[f].CalculateValue(R);
                  console.log(tmp + " -> " + R.id);
                  mindex.push(tmp);
               } else {
                  mindex.push(0);
               }
      ...
      _renderHtml: function() {
         ...
         S += "<div class='col-sm-3 col-md-2 sidebar'>";
         
         S += "<div class='dropable targetU0' rel='U,0'>";
         S += "<div class='panel panel-primary'>";
         S += "<div class='panel-heading'>Dimensions</div>";
         S += "<div class='list-group'>";
         for (i = 0; i < this.ufields.length; i++) {
            cls = "draggable dropable";
            cls += " targetU" + (i + 1);
            
            if(this.options.fields[this.fieldNames[this.ufields[i]]].type == 'dimension') {
          		S+= "<a href='#' class='list-group-item " + cls + "'" + " rel= 'U," + (i + 1) + "'" + ">";
         		S+= this.fieldLabels[this.ufields[i]] + "</a>";
            }
         ...
         S += "<div class='dropable targetU0' rel='U,0'>";
         S += "<div class='panel panel-primary'>";
         S += "<div class='panel-heading'>Measures</div>";
         S += "<div class='list-group'>";
         for (i = 0; i < this.ufields.length; i++) {
            cls = "draggable dropable";
            cls += " targetU" + (i + 1);
            
            if(this.options.fields[this.fieldNames[this.ufields[i]]].type == 'measure') {
          		S+= "<a href='#' class='list-group-item " + cls + "'" + " rel= 'U," + (i + 1) + "'" + ">";
         		S+= this.fieldLabels[this.ufields[i]] + "</a>";
            }
        ...
        // S += "</table>";
		 ...
		 S += "<h2 class='sub-header'>Pivot View</h2>"
         ...
         if (this.yfields.length > 0) {
            //Pradeep: Remove collapse button
